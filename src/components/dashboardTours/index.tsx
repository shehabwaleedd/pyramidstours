...
const DashboardTours = ({ tours, loading, title }: { tours: TourType[], loading: boolean, title: string }) => {
    ...
    const saveOrder = async () => {
        const orderedNews = currentTours.map((item: any, index: number) => ({ _id: item._id, index: index + 1 }));
        try {
                                    ...
                                    {(provided) => (
                                        <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className={styles.userTours__container_card}>
                                            ...
                                            <div className={styles.btns}>
                                                <button onClick={() => handleEditClick(event.title)} style={{ backgroundColor: "#2e2e2e", color: "var(--container-color)" }}>
                                                    <span >
                                                        Edit
                                                    </span>
                                                </button>
